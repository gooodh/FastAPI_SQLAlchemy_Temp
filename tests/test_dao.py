...
class TestBaseDAO:
    ...
    @pytest.mark.asyncio
    async def test_find_one_or_none_with_filters(self, db_session: AsyncSession, test_user):
    ...
    @pytest.mark.asyncio
    async def test_find_all_without_filters(self, db_session: AsyncSession, test_user, test_admin_user):
    ...
    async def test_delete_record(self, db_session: AsyncSession):
        ...
        # Удаляем роль
        deleted_count = await role_dao.delete(
            filters=RoleAddModel(name="to_delete")
        )
        await db_session.commit()
    ...
    @pytest.mark.asyncio
    async def test_count_records(self, db_session: AsyncSession, test_user, test_admin_user):
    ...
    async def test_bulk_update(self, db_session: AsyncSession, test_role):
        ...
        for user in created_users:
            update_data.append(BulkUpdateModel(
                id=user.id,
