...
from app.auth.schemas import SUserAddDB, RoleAddModel
...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_bulk_user_creation_performance(self, db_session: AsyncSession, test_role):
    ...
    async def test_concurrent_user_registration(self, client: AsyncClient, test_role):
        ...
        print(f"20 одновременных регистраций: {successful_registrations}/20 успешных за {execution_time:.2f} секунд")

    @pytest.mark.asyncio
    async def test_database_query_performance(self, db_session: AsyncSession, test_role):
        ...
        assert avg_query_time < 0.1  # Каждый запрос должен выполняться менее чем за 100мс
    ...
    async def test_password_hashing_performance(self):
        ...
        assert all(len(h) > 50 for h in hashes)  # Все хеши должны быть достаточно длинными
    ...
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_memory_usage_bulk_operations(self, db_session: AsyncSession, test_role):
...
class TestLoadTesting:
    ...
    async def test_stress_user_creation(self, client: AsyncClient, test_role):
        ...
        async def create_user_batch(batch_id: int, batch_size: int = 5):  # Уменьшаем размер батча
            successful = 0
        ...
        print(f"Стресс-тест: {total_successful}/9 пользователей создано за {execution_time:.2f} секунд")